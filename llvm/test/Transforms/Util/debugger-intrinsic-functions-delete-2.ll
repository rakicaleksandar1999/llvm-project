...
; CHECK-NEXT: {{^}}define dso_local i32 @mult() #0 !dbg !10 {{{$}}
; CHECK-NEXT: {{^}}entry:{{$}}
; CHECK-NEXT: {{^}}  %a = alloca i32, align 4{{$}}
; CHECK-NEXT: {{^}}  %b = alloca i32, align 4{{$}}
; CHECK-NEXT: {{^}}  %c = alloca i32, align 4{{$}}
; CHECK-NEXT: {{^}}  store i32 5, ptr %a, align 4, !dbg !15{{$}}
; CHECK-NEXT: {{^}}  store i32 3, ptr %b, align 4, !dbg !16{{$}}
; CHECK-NEXT: {{^}}  %0 = load i32, ptr %a, align 4, !dbg !17{{$}}
; CHECK-NEXT: {{^}}  %1 = load i32, ptr %b, align 4, !dbg !18{{$}}
; CHECK-NEXT: {{^}}  %mul = mul nsw i32 %0, %1, !dbg !19{{$}}
; CHECK-NEXT: {{^}}  store i32 %mul, ptr %c, align 4, !dbg !20{{$}}
; CHECK-NEXT: {{^}}  %2 = load i32, ptr %b, align 4, !dbg !21{{$}}
; CHECK-NEXT: {{^}}  %add = add nsw i32 %2, 5, !dbg !22{{$}}
; CHECK-NEXT: {{^}}  store i32 %add, ptr %a, align 4, !dbg !23{{$}}
; CHECK-NEXT: {{^}}  %3 = load i32, ptr %c, align 4, !dbg !24{{$}}
; CHECK-NEXT: {{^}}  ret i32 %3, !dbg !25{{$}}
; CHECK-NEXT: {{^}}}{{$}}
